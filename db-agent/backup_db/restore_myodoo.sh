#!/bin/bash
# restore_myodoo.sh — Restore the myodoo (Odoo) database from a gzipped SQL backup.
# Run INSIDE mypostgresql_db-container.
# Usage: bash restore_myodoo.sh <backup_file.sql.gz>
set -euo pipefail
...
if [ -z "$BACKUP_FILE" ]; then
    echo "[ERROR] Usage: bash restore_myodoo.sh <backup_file.sql.gz>"
    exit 1
fi
...
echo "[INFO] Restoring $ODOO_DB from $BACKUP_FILE..."
gunzip -c "$BACKUP_FILE" | psql -U "$ODOO_USER" -h "$PG_HOST" -p "$PG_PORT" "$ODOO_DB"
